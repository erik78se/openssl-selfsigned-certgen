...
_PATH="certs"
SERVER_KEY="$_PATH/server.key"
SERVER_CSR="$_PATH/server.csr"
SERVER_CRT="$_PATH/server.crt"
FULLCHAIN_PEM="$_PATH/fullchain.pem"
...
mkdir -p $_PATH
if [ ! -d $_PATH ]; then
   echo "Failed creating output path $_PATH"
   exit 1
fi
...
echo "Assembling fullchain.pem from key and cert"
cat $SERVER_CRT $SERVER_KEY > $FULLCHAIN_PEM
